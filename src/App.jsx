...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn { transition: transform 0.15s, filter 0.15s; cursor: pointer; -webkit-tap-highlight-color: transparent; touch-action: manipulation; }
    .btn:active { transform: scale(0.94); filter: brightness(0.88); }
    .card-hover { transition: all 0.2s; }
    .card-hover:active { transform: scale(0.97); filter: brightness(0.92); }
    ...
    .skeleton { background: rgba(255,255,255,0.08); border-radius: 8px; animation: skeletonPulse 1.4s ease-in-out infinite; }
    .tab-content { animation: tabFade 0.22s ease forwards; }
  `}</style>
);
...
// -- CONFETTI --
const CONFETTI_COLORS = ["#c9a96e","#f0d080","#a78bfa","#e879f9","#34d399","#67e8f9","#f87171","#fff"];
...
const usePullToRefresh = (onRefresh) => {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      ...
      if (dy > 0) setIndicator(Math.min(1, dy / 80));
    };
    const onTouchEnd = () => {
      if (indicator >= 1) onRefresh();
      setIndicator(0); pulling.current = false;
    };
...
const PullIndicator = ({ progress }) => {
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", justifyContent: "center", paddingTop: 12, pointerEvents: "none" }}>
      <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 99, padding: "6px 16px", display: "flex", alignItems: "center", gap: 8, backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.1)", transform: `scale(${0.7 + progress * 0.3})`, transition: "transform 0.1s" }}>
        <div style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.2)", borderTop: `2px solid ${progress >= 1 ? "#34d399" : "rgba(255,255,255,0.6)"}`, borderRadius: "50%", animation: progress >= 1 ? "float 0.6s linear infinite" : "none", transform: `rotate(${progress * 360}deg)` }} />
        <div style={{ fontSize: 11, color: progress >= 1 ? "#34d399" : "rgba(255,255,255,0.5)", letterSpacing: 1 }}>{progress >= 1 ? "RELEASE" : "PULL TO REFRESH"}</div>
      </div>
    </div>
  );
};
...
// -- SKELETON LOADER --
const SkeletonChore = ({ accent = "rgba(255,255,255,0.08)" }) => (
...
// -- EMPTY STATE --
const EmptyState = ({ emoji, title, subtitle, color = "rgba(255,255,255,0.15)" }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "52px 24px", gap: 14 }}>
    <div style={{ fontSize: 52, filter: "grayscale(0.3)", animation: "float 3s ease-in-out infinite" }}>{emoji}</div>
    <div style={{ fontSize: 15, color, textAlign: "center", fontStyle: "italic" }}>{title}</div>
    {subtitle && <div style={{ fontSize: 12, color: "rgba(255,255,255,0.2)", textAlign: "center" }}>{subtitle}</div>}
  </div>
);
...
const LoginScreen = ({ onLogin, pins, setPins }) => {
  ...
  const needsSetup = selected && !pins[selected] && !pins.dad;

  const handleLoginSuccess = (mode) => {
    // Only force PIN change if the kid's actual stored PIN is the temp pin
    // (meaning Dad reset it). If they have no PIN yet, they already went through setup.
    // If Dad used his master PIN to log in as a kid whose PIN is temp, force change.
...
const HistoryModal = ({ kid, accentColor, onClose }) => {
  ...
  const onDragMove = (e) => { const dy = (e.touches ? e.touches[0].clientY : e.clientY) - startY.current; if (dy > 0) setDragY(dy); };
  const onDragEnd = () => { if (dragY > 80) onClose(); else setDragY(0); };
...
const WorkStudy = ({ balances, setBalancesLocal }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {tasks.length === 0 && <EmptyState emoji="⏰" title="No work study tasks yet" subtitle="Add a task above to get started" />}
      {tasks.map(task => (
...
const DadDashboard = ({ onLogout, pins, setPins }) => {
  ...
  const [withdrawForm, setWithdrawForm] = useState({ kid: "alan", amount: "", note: "" });
  ...
  const saveEditChore = async (kid, id, name, value) => {
    await updateDoc(doc(db, `chores_${kid}`, id), { name, value: parseFloat(value) });
    setEditingChore(null);
  };

  const resetAllChores = async (kid) => {
    const batch = chores[kid].filter(c => c.done);
    await Promise.all(batch.map(c => updateDoc(doc(db, `chores_${kid}`, c.id), { done: false })));
  };

  const handleWithdraw = async () => {
    if (!withdrawForm.amount || parseFloat(withdrawForm.amount) <= 0) return;
    const amt = parseFloat(withdrawForm.amount);
    const kid = withdrawForm.kid;
    const newBal = { ...balances, [kid]: Math.max(0, (balances[kid] || 0) - amt) };
    await setDoc(doc(db, "balances", "main"), newBal);
    await addDoc(collection(db, `history_${kid}`), {
      choreName: `Withdrawal: ${withdrawForm.note || "Balance adjustment"}`,
      value: -amt, photo: null, approvedAt: Date.now(),
    });
    setWithdrawForm({ kid: "alan", amount: "", note: "" });
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", overflowX: "hidden", background: "linear-gradient(160deg, #0d1120 0%, #1a1a2e 50%, #0d1120 100%)", fontFamily: "'DM Sans', sans-serif", color: "#fff" }}>
      ...
      {showWithdraw && (
        <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowWithdraw(false)}>
          <div style={{ background: "#1a1a2e", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 340 }} onClick={e => e.stopPropagation()}>
            <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 18, color: "#fff", marginBottom: 4 }}>Withdraw Funds</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginBottom: 20 }}>Deduct from a kid's balance (e.g. they bought their wish item)</div>
            <select value={withdrawForm.kid} onChange={e => setWithdrawForm(p => ({ ...p, kid: e.target.value }))} style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "10px 14px", borderRadius: 10, fontSize: 14, outline: "none", marginBottom: 10 }}>
              <option value="alan">Alan (${(balances.alan||0).toFixed(2)})</option>
              <option value="zelda">Zelda (${(balances.zelda||0).toFixed(2)})</option>
            </select>
            <input placeholder="Amount to deduct" type="number" step="0.01" value={withdrawForm.amount} onChange={e => setWithdrawForm(p => ({ ...p, amount: e.target.value }))} style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "10px 14px", borderRadius: 10, fontSize: 14, outline: "none", marginBottom: 10 }} />
            <input placeholder="Reason (e.g. Bought wish item)" value={withdrawForm.note} onChange={e => setWithdrawForm(p => ({ ...p, note: e.target.value }))} style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "10px 14px", borderRadius: 10, fontSize: 14, outline: "none", marginBottom: 16 }} />
            <div style={{ display: "flex", gap: 8 }}>
              <button className="btn" onClick={handleWithdraw} style={{ flex: 1, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#f87171", padding: "10px", borderRadius: 11, cursor: "pointer" }}>Deduct</button>
              <button onClick={() => setShowWithdraw(false)} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.4)", padding: "10px 14px", borderRadius: 11, cursor: "pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(200,160,80,0.15)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(0,0,0,0.35)", backdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          ...
          <button className="btn" onClick={() => setShowWithdraw(true)} style={{ background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.2)", color: "rgba(248,113,113,0.7)", padding: "7px 12px", borderRadius: 9, cursor: "pointer", fontSize: 11 }}>-$</button>
              ...
              <div key={kid}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    {chores[kid].some(c => c.done) && <button className="btn" onClick={() => resetAllChores(kid)} style={{ background: "none", border: "none", color: "rgba(255,160,80,0.55)", fontSize: 10, cursor: "pointer", letterSpacing: 1 }}>RESET ALL</button>}
                ...
                {chores[kid].length === 0 && <EmptyState emoji="📝" title="No chores yet" subtitle={`Add one above for ${kid}`} />}
                {chores[kid].map(chore => (
                  <div key={chore.id} className="fade-in" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", paddingBottom: 10, marginBottom: 10 }}>
                    {editingChore?.id === chore.id && editingChore?.kid === kid ? (
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <input defaultValue={chore.name} onChange={e => setEditingChore(p => ({ ...p, name: e.target.value }))} style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "7px 10px", borderRadius: 8, fontSize: 13, outline: "none", minWidth: 0 }} />
                        <input defaultValue={chore.value} type="number" onChange={e => setEditingChore(p => ({ ...p, value: e.target.value }))} style={{ width: 70, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", padding: "7px 10px", borderRadius: 8, fontSize: 13, outline: "none" }} />
                        <button className="btn" onClick={() => saveEditChore(kid, chore.id, editingChore.name, editingChore.value)} style={{ background: "rgba(52,211,153,0.12)", border: "1px solid rgba(52,211,153,0.3)", color: "#34d399", padding: "7px 12px", borderRadius: 8, cursor: "pointer", fontSize: 12 }}>Save</button>
                        <button onClick={() => setEditingChore(null)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: 16 }}>x</button>
                      </div>
                    ) : (
                    ...
                    <button className="btn" onClick={() => setEditingChore({ id: chore.id, kid, name: chore.name, value: chore.value })} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.25)", cursor: "pointer", fontSize: 12, flexShrink: 0, padding: "4px 6px" }}>edit</button>
...
const ZeldaWorld = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    unsubs.push(onSnapshot(collection(db, "chores_zelda"), snap => {
      ...
      setChores([...all.filter(c => !c.done), ...all.filter(c => c.done)]);
  ...
  const pullProgress = usePullToRefresh(() => window.location.reload());
  return (
      ...
      <div style={{ padding: "0 16px 60px", maxWidth: 500, margin: "0 auto", position: "relative", zIndex: 10 }}>
        {tab === "chores" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {chores.length === 0 && !choresLoading && <EmptyState emoji="✨" title="No quests yet!" subtitle="Ask Dad to add some" color="rgba(167,139,250,0.5)" />}
            {chores.map((chore, i) => {
              const showDoneDivider = i > 0 && chore.done && !chores[i-1].done;
              return (<div key={chore.id}>
              {showDoneDivider && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0 14px", opacity: 0.5 }}>
                  <div style={{ flex: 1, height: 1, background: "rgba(167,139,250,0.2)" }} />
                  <div style={{ fontSize: 9, color: "rgba(167,139,250,0.5)", letterSpacing: 2 }}>COMPLETED</div>
                  <div style={{ flex: 1, height: 1, background: "rgba(167,139,250,0.2)" }} />
                </div>
              )}
              <div style={{
        ...
        {tab === "wishlist" && (
          <div key="wishlist" className="tab-content" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", overflowX: "hidden", background: "#0d0d0d", fontFamily: "'DM Sans', sans-serif", color: "#fff" }}>
      ...
      <div style={{ padding: "24px 16px", maxWidth: 600, margin: "0 auto" }}>
        {tab === "chores" && (
          <div>
            {choresLoading && [1,2,3].map(i => <SkeletonChore key={i} />)}
            {!choresLoading && chores.length===0 && <EmptyState emoji="📋" title="No chores assigned yet" subtitle="Check back after Dad adds some" />}
            {chores.map((chore, i) => {
              const showDoneDivider = i > 0 && chore.done && !chores[i-1].done;
              return (<div key={chore.id}>
              {showDoneDivider && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0 14px", opacity: 0.5 }}>
                  ...
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)", letterSpacing: 2 }}>COMPLETED</div>
                  <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.08)" }} />
                </div>
              )}
              <div style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", paddingBottom: 16, marginBottom: 16, animation: `slideIn 0.3s ease ${i*0.05}s both` }}>
                {chore.refPhoto && (
                  <div style={{ marginBottom: 10 }}>
        ...
        {tab === "reality" && (
          <div key="reality" className="tab-content">
