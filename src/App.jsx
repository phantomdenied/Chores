...
const scrapeAmazon = async (inputUrl) => {
  // Step 1: resolve short URLs (a.co) via allorigins
  let resolvedUrl = inputUrl;
  try {
    const r = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(inputUrl)}`);
    const j = await r.json();
    if (j.status?.url) resolvedUrl = j.status.url;
    // Also try to find full amazon URL in redirect chain
    const match = (j.status?.url || "").match(/amazon\.com[^\s"']*/);
    if (match) resolvedUrl = "https://" + match[0];
  } catch {}

  // Step 2: try to get ASIN and use Open Graph / product API
  const asin = extractAsin(resolvedUrl) || extractAsin(inputUrl);

  // Step 3: fetch the product page via allorigins
  ...
  const title = d.querySelector("#productTitle")?.textContent?.trim() ||
    ...
    d.querySelector(".product-title")?.textContent?.trim() ||
  ...
  const price = priceStr ? parseFloat(priceStr) : (priceWhole ? parseFloat(`${priceWhole}.${priceFrac}`) : 0);

  // Image - try OG image first (most reliable)
  const img = d.querySelector("meta[property='og:image']")?.getAttribute("content") ||
    ...
    (asin ? `https://ws-na.amazon-adsystem.com/widgets/q?_encoding=UTF8&ASIN=${asin}&Format=_SL250_&ID=AsinImage&MarketPlace=US&ServiceVersion=20070822&WS=1` : null);
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    html, body { width: 100%; max-width: 100vw; overflow-x: hidden; overflow-y: auto; background: #0a0a0a; position: relative; }
    #root { width: 100%; max-width: 100vw; overflow-x: hidden; }
    * { -webkit-overflow-scrolling: touch; }
    body { overscroll-behavior-x: none; position: fixed; width: 100%; }
